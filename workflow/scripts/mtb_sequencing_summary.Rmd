...
```{r setup}
root_dir = '/labs/jandr/walter/tb/mtb/'
working_dir = paste('/labs/jandr/walter/tb/mtb/', sep = '')
...
```{r read in combined stats}
setwd(working_dir)
filename = 'working/combined_all_stats_090222.csv'
#filename = 'working/combined_all_stats_hdr_090622.csv'
df <- read_csv(filename) %>% filter(!str_starts(unclassified_per, 'unclassified')) %>% select(!sampl...43) %>%
  filter(!batch == '10') # incorrect file from weiler set of seqs.

df <- df %>% 
  mutate(across(!sampl...1 & !sample & !batch & !fastq_1 & !sample & !refname, as.double))

# Rename some of the batches
unique(df$batch)
df <- df %>%
  mutate(batch = case_when(batch %in% c( "Batch_2018-06-26", "Batch_2018-01-11") ~ 'IS-1045', TRUE ~ batch))

```
```{r plot mean coverage by batch }
df %>%
  group_by(batch) %>%
  ggplot(aes(x = batch, y = MEAN_COVERAGE)) + 
    geom_bar(position = "dodge",
           stat = "summary",
           fun = "mean") + 
  theme_classic() + xlab('Batch') + ylab('Mean coverage') + 
  theme(axis.text.x = element_text(angle = 90))

# 1800 sequences with > 40X mean coverage.
df %>%
  filter(MEAN_COVERAGE > 40)
```
...
df %>%
  ...
  select(c(batch, 'raw_reads','trim_reads','kraken_reads', 'raw_bases','trim_bases','kraken_bases')) %>%
  pivot_longer(cols = c('raw_reads','trim_reads','kraken_reads', 'raw_bases','trim_bases','kraken_bases'), names_to = 'type', values_to = 'reads') %>%
