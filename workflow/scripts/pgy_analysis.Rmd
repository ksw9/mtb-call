...
meta_filename = '/labs/jandr/walter/tb/pgy/metadata/pgy_metadata_080822.csv'
...
sra_metadata_pgy <- pgy_sra %>%
  select(sample_name, title, library_ID,library_strategy, library_source, library_selection, library_layout, platform, instrument_model, design_description, filetype, filename, filename2)

write_tsv(sra_metadata_pgy, file = paste(meta_dir,'pgy_biosample_data_081222.tsv', sep = ''))
...
### HERE ###
# Get acession numbers of samples included in analysis.
sra_spillover <-sra %>%
...
# Write metadata for the Supplement.
df_si <- df %>% filter(include == 1) %>%
  dplyr::select(seq_name, sex, cidade, date, inc_status_combined)
write.csv(df_si, file = '~/Box/Box/TB/spillover/seq/sra/sample_metadata.csv', row.names = FALSE)
